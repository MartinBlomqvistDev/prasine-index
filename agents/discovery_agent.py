...
def extract_relevant_links(html: str, base_url: str, max_links: int = 5) -> list[str]:
    ...
    # Enforce path-prefix diversity: cap at 4 URLs per 2-segment path prefix.
    # 4 (not 2) allows article/blog-heavy entry points (e.g. stories/articles/*)
    # to surface enough distinct items while still preventing a single theme from
    # monopolising all slots when a site has redundant subpage variations.
